...
**Status**: `proposal` (승인 시 F-SYNC-001 / F-SPACE-001 / F-PLAN-001 / F-ENT-001 / F-RESTORE-001 로 분해)
**작성일**: 2026-09-19
...
| Phase | 스펙 | 내용 | 예상 |
|---|---|---|---|
| S1 | F-SYNC-001 | schema v2 + `snapshots` PUT/GET + `merge.ts` + `summarize.ts` + 파일 내보내기/가져오기. 가정(P-B) 복원 완성 | 3 d |
| S2 | F-RESTORE-001 | Rescue Code 생성·표시·claim + rate limit + 이메일 | 1.5 d |
...
1. **학교 consent 모드**: 한글학교(미국 기준 COPPA) 에서 교사가 학교 동의를 대신 attest 하는 흐름을 허용할지, 항상 부모 이메일을 받을지. 법률 검토 항목 (BP03 §"법률 검토").
2. **Rescue Code 를 기본 ON 으로 할지** (모든 아이가 첫 업로드 때 자동 생성) vs 부모가 켤 때만. 제안: 기본 ON — "혼자 하는 아이" 가 앱을 지웠을 때가 가장 흔한 데이터 손실 케이스.
3. **텔레메트리 `events` 를 D1 에 둘지 Analytics Engine 으로 옮길지** — write 예산의 대부분을 먹는 것이 이 테이블이다.
4. **교사 Free 캡 20명** 유지 vs 30명 (한글학교 평균 학급 크기 확인 필요, docs/beta 사이클에서 측정).
5. **가격**: family $5.99 / $39.99 는 가설. PH 런치 후 A/B.
